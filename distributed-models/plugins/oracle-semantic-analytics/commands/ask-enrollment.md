...
5. Generate relevant semantic context with `<plugin-root>/scripts/run_tool.py generate_prompt_context.py`.
6. Ask clarifying questions if the term, metric, or grouping is ambiguous.
7. Generate Oracle 19c SELECT-only SQL.
8. Validate immediately with `<plugin-root>/scripts/run_tool.py validate_sql.py -` using stdin; do not ask before running local validation.
...
11. Execute only with `<plugin-root>/scripts/run_tool.py execute_oracle_readonly.py - --yes` using stdin.
